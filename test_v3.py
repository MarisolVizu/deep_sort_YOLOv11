...
# ===== CONFIGURACIÓN =====
VIDEO_INPUT = "video_generado11.avi"
...
USE_RESIZE = False  ## Si True, redimensiona frames para video de salida
# Inicializar YOLO
...
# ===== PREPARAR CSV =====
csv_data = []
csv_headers = ["frame", "track_id", "x1", "y1", "x2", "y2", "confidence", "class"]
...
video_writer = None
if VIDEO_OUTPUT:
    # Usar tamaño apropiado según configuración
    if USE_RESIZE:
        out_width = int(width * 1.2)
        out_height = int(height * 1.2)
    else:
        out_width = width
        out_height = height
    ...
    video_writer = cv2.VideoWriter(VIDEO_OUTPUT, fourcc, fps, (out_width, out_height))
    
    if video_writer.isOpened():
        print(f"💾 Video de salida: {VIDEO_OUTPUT} ({out_width}x{out_height}) - Codec: {codec_name}")
    else:
...
while frame_idx < frames_to_process:
    ...
    frame_enhanced = cv2.detailEnhance(frame_enhanced, sigma_s=12, sigma_r=0.15)
    frame_enhanced = cv2.bilateralFilter(frame_enhanced, 5, 40, 40)
    ...
    if len(r.boxes):
        ...
        if len(boxes_cars) > 0:
            ...
            # 🎯 Actualizar DeepSORT (retorna outputs y mask_outputs)
            ...
            if len(outputs) > 0:
                ...
                for output in outputs:
                    ...
                    # Buscar la confianza correspondiente (aproximación)
                ...
                if video_writer and video_writer.isOpened():
                    ...
                    if annotated.shape[:2] != (out_height, out_width):
                        annotated_resized = cv2.resize(annotated, (out_width, out_height))
                        video_writer.write(annotated_resized)
                    else:
                        video_writer.write(annotated)
            else:
                if video_writer and video_writer.isOpened():
                    if frame_enhanced.shape[:2] != (out_height, out_width):
                        frame_resized = cv2.resize(frame_enhanced, (out_width, out_height))
                        video_writer.write(frame_resized)
                    else:
                        video_writer.write(frame_enhanced)
        else:
            if video_writer and video_writer.isOpened():
                if frame_enhanced.shape[:2] != (out_height, out_width):
                    frame_resized = cv2.resize(frame_enhanced, (out_width, out_height))
